...
async def verify_email(request: VerifyEmailRequest):
    ...
    verifications = _get_verification_table()
    res = verifications.select("code,expires_at,name,password_hash").eq("email", email).order("id", desc=True).limit(1).execute()
    if not res.data:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def resend_verification(request: ResendVerificationRequest):
    ...
    verifications = _get_verification_table()
    res = verifications.select("code,expires_at,name").eq("email", email).order("id", desc=True).limit(1).execute()
    if not res.data:
        raise HTTPException(
